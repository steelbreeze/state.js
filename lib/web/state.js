...
function invoke(actions, instance, deepHistory, message) {
...
var State = (function (_super) {
    ...
    State.prototype.exit = function (action) {
        this.exitBehavior.push(function (instance, deepHistory, message) {
    ...
    State.prototype.entry = function (action) {
        this.entryBehavior.push(function (instance, deepHistory, message) {
            action(instance, message);
        });
        this.getRoot().clean = false;
        return this;
...
var StateMachine = (function () {
    ...
    StateMachine.prototype.evaluate = function (instance, message) {
        if (this.clean === false) {
            this.initialise();
        }
...
var Transition = (function () {
    function Transition(source, target, kind) {
        ...
        this.guard = source instanceof PseudoState ? function (instance, message) { return true; } : function (instance, message) { return message === _this.source; };
    ...
    Transition.prototype.effect = function (action) {
        this.effectBehavior.push(function (instance, deepHistory, message) {
            action(instance, message);
        });
        this.source.getRoot().clean = false;
        return this;
...
Transition.Else = function (instance, message) { return false; };
...
var InitialiseStateMachine = (function (_super) {
    ...
    InitialiseStateMachine.prototype.visitElement = function (element, deepHistoryAbove) {
        this.getActions(element).leave.push(function (instance, deepHistory, message) { return exports.logger.log(instance + " leave " + element); });
        this.getActions(element).beginEnter.push(function (instance, deepHistory, message) { return exports.logger.log(instance + " enter " + element); });
    };
    InitialiseStateMachine.prototype.visitRegion = function (region, deepHistoryAbove) {
        ...
        this.getActions(region).leave.push(function (instance, deepHistory, message) {
            var currentState = instance.getCurrent(region);
            if (currentState) {
                invoke(_this.getActions(currentState).leave, instance, false, message);
            }
        });
        ...
        if (deepHistoryAbove || !regionInitial || regionInitial.isHistory()) {
            this.getActions(region).endEnter.push(function (instance, deepHistory, message) {
    ...
    InitialiseStateMachine.prototype.visitVertex = function (vertex, deepHistoryAbove) {
        ...
        this.getActions(vertex).beginEnter.push(function (instance, deepHistory, message) {
    ...
    InitialiseStateMachine.prototype.visitPseudoState = function (pseudoState, deepHistoryAbove) {
        ...
        if (pseudoState.isInitial()) {
            this.getActions(pseudoState).endEnter.push(function (instance, deepHistory, message) {
                if (instance.getLastKnownState(pseudoState.parent)) {
                    invoke(_this.getActions(pseudoState).leave, instance, false, message);
                    var currentState = instance.getLastKnownState(pseudoState.parent);
                    if (currentState) {
                        invoke(_this.getActions(currentState).beginEnter, instance, deepHistory || pseudoState.kind === PseudoStateKind.DeepHistory, message);
                        invoke(_this.getActions(currentState).endEnter, instance, deepHistory || pseudoState.kind === PseudoStateKind.DeepHistory, message);
                    }
                }
                else {
    ...
    InitialiseStateMachine.prototype.visitInternalTransition = function (transition) {
        ...
        if (exports.internalTransitionsTriggerCompletion) {
            transition.onTraverse.push(function (instance, deepHistory, message) {
    ...
    InitialiseStateMachine.prototype.visitLocalTransition = function (transition) {
        ...
        transition.onTraverse.push(function (instance, deepHistory, message) {
            ...
            invoke(_this.getActions(firstToExit).leave, instance, false, message);
            invoke(transition.effectBehavior, instance, false, message);
            while (i < targetAncestors.length) {
                invoke(_this.getActions(targetAncestors[i++]).beginEnter, instance, false, message);
            }
            invoke(_this.getActions(transition.target).endEnter, instance, false, message);
        });
    };
...
function selectTransition(pseudoState, instance, message) {
    var transitions = pseudoState.outgoing.filter(function (transition) { return transition.guard(instance, message); });
...
function evaluate(state, instance, message) {
    ...
    if (state instanceof State) {
        if (result) {
            ...
            var transitions = state.outgoing.filter(function (transition) { return transition.guard(instance, message); });
            if (transitions.length === 1) {
