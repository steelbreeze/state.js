...
var InitialiseStateMachine = (function (_super) {
    ...
    InitialiseStateMachine.prototype.visitExternalTransition = function (transition) {
        ...
        var targetAncestors = Tree.Ancestors(transition.target);
        console.log("source = " + transition.source);
        console.log("target = " + transition.target);
        console.log("sourceAncestors.length = " + sourceAncestors.length);
        console.log("targetAncestors.length = " + targetAncestors.length);
        // let i = Math.min(sourceAncestors.length, targetAncestors.length) - 1;
        // while (sourceAncestors[i] !== targetAncestors[i]) { i -= 1; } // TODO: replace with Tree.LCA
        var i = Tree.LCA(sourceAncestors, targetAncestors);
