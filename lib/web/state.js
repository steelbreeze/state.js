...
var InitialiseStateMachine = (function (_super) {
    ...
    InitialiseStateMachine.prototype.visitLocalTransition = function (transition) {
        ...
        transition.onTraverse.push(function (instance, deepHistory) {
            ...
            var i = Tree.lowestCommonAncestorIndex(sourceAncestors, targetAncestors) + 2; // NOTE: we do not leave the source state, so entry/exit from the next state below
...
function ancestors(node) {
    var result = node && node.parent ? ancestors(node.parent) : new Array();
    if (node) {
        result.push(node);
    }
    return result;
