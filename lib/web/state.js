...
var StateMachine = (function () {
    ...
    function StateMachine(name) {
         ...
         * @hidden
         */
        this.onInitialise = delegate_1.delegate();
    }
    /**
     * Invalidates a [state machine model]{@link StateMachine} causing it to require recompilation.
     * @hidden
    ...
    StateMachine.prototype.initialise = function (instance) {
        if (instance) {
            if (this.onInitialise === delegate_1.delegate()) {
                this.initialise();
            }
...
var InitialiseStateMachine = (function (_super) {
    ...
    InitialiseStateMachine.prototype.visitElement = function (element, deepHistoryAbove) {
        this.getActions(element).leave = delegate_1.delegate(this.getActions(element).leave, function (instance) { return logger.log(instance + " leave " + element); });
        this.getActions(element).beginEnter = delegate_1.delegate(this.getActions(element).beginEnter, function (instance) { return logger.log(instance + " enter " + element); });
    };
