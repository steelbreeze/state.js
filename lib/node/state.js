...
var Vertex = (function (_super) {
    ...
    function Vertex(name, parent) {
        _super.call(this, name, parent instanceof State ? parent.getDefaultRegion() : parent);
...
function isComplete(stateOrRegion, instance) {
    if (stateOrRegion instanceof Region) {
        var currentState = instance.getCurrent(stateOrRegion);
...
/** @internal */ var Behavior = (function () {
    function Behavior() {
        ...
        this.beginLeave = new Array();
        this.endLeave = new Array();
    ...
    Behavior.prototype.leave = function () {
        var result = new Array();
        push(result, this.beginLeave, this.endLeave);
        return result;
    };
...
/** @internal */ var InitialiseTransitions = (function (_super) {
    ...
    InitialiseTransitions.prototype.visitLocalTransition = function (transition, behavior) {
        ...
        transition.onTraverse.push(function (message, instance) {
            ...
            var currentState = instance.getCurrent(targetAncestors[i].parent);
            if (currentState) {
            ...
            while (i < targetAncestors.length) {
                _this.cascadeElementEntry(behavior, targetAncestors[i++], targetAncestors[i], function (behavior) { return invoke(behavior, message, instance); });
            }
            // trigger cascade
    ...
    InitialiseTransitions.prototype.visitExternalTransition = function (transition, behavior) {
        ...
        var j = sourceAncestors.length - 1;
        var i = Math.min(j, targetAncestors.length - 1);
        ...
        while (j-- > i) {
            ...
            this.cascadeElementLeave(behavior, sourceAncestors[j], sourceAncestors[j], function (behavior) { return push(transition.onTraverse, behavior); });
        }
        push(transition.onTraverse, transition.transitionBehavior);
        // enter the target ancestry
        while (i < targetAncestors.length) {
            this.cascadeElementEntry(behavior, targetAncestors[i++], targetAncestors[i], function (behavior) { return push(transition.onTraverse, behavior); });
        }
        // trigger cascade
        push(transition.onTraverse, behavior(transition.target).endEnter);
        //        console.log("- endEnter " + transition.target);
    };
    InitialiseTransitions.prototype.cascadeElementLeave = function (behavior, vertex, next, task) {
    ...
    InitialiseTransitions.prototype.cascadeElementEntry = function (behavior, vertex, next, task) {
...
/** @internal */ var InitialiseElements = (function (_super) {
    ...
    InitialiseElements.prototype.visitNamedElement = function (namedElement, deepHistoryAbove) {
        this.behavior(namedElement).beginLeave.push(function (message, instance) { return exports.console.log(instance + " leave " + namedElement.qualifiedName); });
    ...
    InitialiseElements.prototype.visitState = function (state, deepHistoryAbove) {
        ...
        for (var _i = 0, _a = state.regions; _i < _a.length; _i++) {
            ...
            push(this.behavior(state).endLeave, this.behavior(region).leave());
        ...
        push(this.behavior(state).beginLeave, state.exitBehavior);
