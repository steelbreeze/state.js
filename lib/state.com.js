...
// invokes behavior
function invoke(to, message, instance, history) {
...
var InitialiseElements = (function (_super) {
    ...
    InitialiseElements.prototype.visitRegion = function (region, deepHistoryAbove) {
        ...
        if (deepHistoryAbove || !regionInitial || regionInitial.isHistory()) {
            this.behavior(region).endEnter.push(function (message, instance, history) { return invoke((_this.behavior((history || regionInitial.isHistory()) ? instance.getCurrent(region) || regionInitial : regionInitial)).enter(), message, instance, history || regionInitial.kind === PseudoStateKind.DeepHistory); });
        }
        else {
        ...
        if (pseudoState.isInitial()) {
            this.behavior(pseudoState).endEnter.push(function (message, instance, history) {
                if (instance.getCurrent(pseudoState.region)) {
                    ...
                    invoke(_this.behavior(instance.getCurrent(pseudoState.region)).enter(), message, instance, history || pseudoState.kind === PseudoStateKind.DeepHistory);
                }
                else {
