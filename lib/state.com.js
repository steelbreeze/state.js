...
(function (StateJS) {
    ...
    var State = (function (_super) {
        ...
        function State(name, parent) {
            ...
            /* internal */ this.exitBehavior = new StateJS.Behavior();
            // user defined behavior (via entry method) to execute when entering a state.
            /* internal */ this.entryBehavior = new StateJS.Behavior();
            /**
...
(function (StateJS) {
    ...
    var Transition = (function () {
        ...
        function Transition(source, target, kind) {
            ...
            /*internal*/ this.transitionBehavior = new StateJS.Behavior();
            this.source = source;
            this.target = target;
...
(function (StateJS) {
    ...
    function traverse(transition, instance, message) {
        ...
        var onTraverse = new StateJS.Behavior(tran.onTraverse);
        ...
        if (target) {
            ...
            if (target instanceof StateJS.PseudoState && target.kind === StateJS.PseudoStateKind.Choice) {
                traverse(selectTransition(target, instance, message), instance, message);
            }
            else if (target instanceof StateJS.State && StateJS.isComplete(target, instance)) {
    ...
    var InitialiseTransitions = (function (_super) {
        ...
        InitialiseTransitions.prototype.visitInternalTransition = function (transition, behavior) {
            ...
            if (StateJS.internalTransitionsTriggerCompletion) {
            ...
            if (pseudoState.isInitial()) {
                this.behavior(pseudoState).endEnter.push(function (message, instance, history) {
                    if (instance.getCurrent(pseudoState.region)) {
                        ...
                        _this.behavior(instance.getCurrent(pseudoState.region)).enter().invoke(message, instance, history || pseudoState.kind === StateJS.PseudoStateKind.DeepHistory);
                    }
                    else {
     ...
     * @member {Boolean}
     */
    StateJS.internalTransitionsTriggerCompletion = false;
})(StateJS || (StateJS = {}));
/*
 * Finite state machine library
 * Copyright (c) 2014-5 Steelbreeze Limited
            ...
            if (pseudoState.kind === StateJS.PseudoStateKind.Choice || pseudoState.kind === StateJS.PseudoStateKind.Junction) {
                ...
                if (pseudoState.isInitial()) {
                    if (pseudoState.outgoing.length > 1) {
                        ...
                        StateJS.console.error(pseudoState + ": initial pseudo states must have no more than one outgoing transition.");
                    }
        ...
        Validator.prototype.visitRegion = function (region) {
            ...
            region.vertices.forEach(function (vertex) {
                if (vertex instanceof StateJS.PseudoState) {
                    if (vertex.kind === StateJS.PseudoStateKind.Initial) {
                        initial++;
                    }
                    else if (vertex.kind === StateJS.PseudoStateKind.DeepHistory) {
                        deepHistory++;
                    }
                    else if (vertex.kind === StateJS.PseudoStateKind.ShallowHistory) {
                        shallowHistory++;
                    }
                }
            });
            ...
            if (deepHistory > 1) {
                StateJS.console.error(region + ": regions may have at most one deep history pseudo state.");
            }
            if (shallowHistory > 1) {
                StateJS.console.error(region + ": regions may have at most one shallow history pseudo state.");
            }
        };
