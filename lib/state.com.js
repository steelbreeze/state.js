...
(function (StateJS) {
    ...
    var InitialiseTransitions = (function (_super) {
        ...
        InitialiseTransitions.prototype.visitExternalTransition = function (transition, behaviour) {
            ...
            // enter the target ancestry
            while (i < targetAncestors.length) {
                this.cascadeElementEntry(transition, behaviour, targetAncestors[i++], i < targetAncestors.length ? targetAncestors[i] : undefined, function (actions) { transition.traverse = transition.traverse.concat(actions); });
            }
            // trigger cascade
