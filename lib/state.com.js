...
(function (StateJS) {
    ...
    var Vertex = (function (_super) {
        ...
        function Vertex(name, parent) {
            _super.call(this, name, parent instanceof StateJS.State ? parent.defaultRegion() : parent);
            /**
             * The set of transitions from this vertex.
             * @member {Array<Transition>}
             */
            this.transitions = [];
            if (this.region = this.parent) {
...
(function (StateJS) {
    ...
    function traverse(transition, instance, message) {
        var transitionBehavior = transition.traverse, target = transition.target;
        ...
        while (target && target instanceof StateJS.PseudoState && target.kind === StateJS.PseudoStateKind.Junction) {
            transition = selectTransition(target, instance, message); // TODO: remove this cast
            target = transition.target;
            Array.prototype.push.apply(transitionBehavior, transition.traverse);
        }
        // execute the transition behaviour
        transitionBehavior.forEach(function (action) { return action(message, instance); });
    ...
    function selectTransition(pseudoState, stateMachineInstance, message) {
        ...
        pseudoState.transitions.forEach(function (transition) {
            if (transition.guard === StateJS.Transition.isElse) {
                if (elseResult) {
    ...
    var InitialiseTransitions = (function (_super) {
        ...
        InitialiseTransitions.prototype.visitLocalTransition = function (transition, behaviour) {
            ...
            transition.traverse.push(function (message, instance) {
                ...
                behaviour[instance.getCurrent(targetAncestors[i].region).qualifiedName].leave.forEach(function (action) { return action(message, instance); }); // TODO: cast
        ...
        InitialiseTransitions.prototype.visitExternalTransition = function (transition, behaviour) {
            ...
            // enter the target ancestry
            while (i < targetAncestors.length) {
                this.cascadeElementEntry(transition, behaviour, targetAncestors[i++], targetAncestors[i], function (actions) { Array.prototype.push.apply(transition.traverse, actions); });
            }
            // trigger cascade
        ...
        InitialiseTransitions.prototype.cascadeElementEntry = function (transition, behaviour, element, next, task) {
            ...
            if (next && element instanceof StateJS.State && element.isOrthogonal()) {
    ...
    var InitialiseElements = (function (_super) {
        ...
        InitialiseElements.prototype.visitElement = function (element, deepHistoryAbove) {
            if (element.getRoot().logTo !== StateJS.defaultConsole) {
                var elementBehaviour = this.behaviour(element);
                elementBehaviour.leave.push(function (message, instance) { return element.getRoot().logTo.log(instance + " leave " + element); });
                elementBehaviour.beginEnter.push(function (message, instance) { return element.getRoot().logTo.log(instance + " enter " + element); });
            }
        };
        InitialiseElements.prototype.visitRegion = function (region, deepHistoryAbove) {
            ...
            region.vertices.forEach(function (vertex) { return vertex.accept(_this, deepHistoryAbove || (region.initial && region.initial.kind === StateJS.PseudoStateKind.DeepHistory)); });
            ...
            if (deepHistoryAbove || !region.initial || region.initial.isHistory()) {
                regionBehaviour.endEnter.push(function (message, stateMachineInstance, history) {
                    _this.behaviour((history || region.initial.isHistory()) ? stateMachineInstance.getCurrent(region) || region.initial : region.initial).enter.forEach(function (action) { return action(message, stateMachineInstance, history || region.initial.kind === StateJS.PseudoStateKind.DeepHistory); });
                });
            }
            else {
        ...
        InitialiseElements.prototype.visitPseudoState = function (pseudoState, deepHistoryAbove) {
            ...
            if (pseudoState.isInitial()) {
                this.behaviour(pseudoState).endEnter.push(function (message, stateMachineInstance) { return traverse(pseudoState.transitions[0], stateMachineInstance); });
            }
