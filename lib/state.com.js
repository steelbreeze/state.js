...
(function (StateJS) {
    function invoke(actions, message, instance, history) {
        ...
        actions.forEach(function (action) { return action(message, instance, history); });
    }
    /**
    ...
    function evaluate(stateMachineModel, stateMachineInstance, message, autoInitialiseModel) {
        if (autoInitialiseModel === void 0) { autoInitialiseModel = true; }
        // log as required
        StateJS.logger.log(stateMachineInstance + " evaluate " + message);
        // initialise the state machine model if necessary
        if (autoInitialiseModel && stateMachineModel.clean === false) {
            initialise(stateMachineModel);
        }
    ...
    function evaluateState(state, stateMachineInstance, message) {
        ...
        if (result) {
            ...
            if (transitions.length === 1) {
                ...
                StateJS.logger.error(state + ": multiple outbound transitions evaluated true for message " + message);
            }
        }
        return result;
    }
    ...
    function selectTransition(pseudoState, stateMachineInstance, message) {
        ...
        if (pseudoState.kind === StateJS.PseudoStateKind.Choice) {
            ...
            if (results.length > 1) {
                StateJS.logger.error("Multiple outbound transition guards returned true at " + this + " for " + message);
            }
            else {
        ...
        InitialiseTransitions.prototype.visitLocalTransition = function (transition, behaviour) {
            ...
            transition.onTraverse.push(function (message, instance) {
                ...
                while (i < targetAncestors.length) {
                    _this.cascadeElementEntry(transition, behaviour, targetAncestors[i++], targetAncestors[i], function (actions) { invoke(actions, message, instance); });
                }
                // trigger cascade
    ...
    var InitialiseElements = (function (_super) {
        ...
        InitialiseElements.prototype.addLogging = function (element, logTo) {
            if (logTo !== defaultLogger) {
                this.behaviour(element).leave.push(function (message, instance) { return logTo.log(instance + " leave " + element); });
                this.behaviour(element).beginEnter.push(function (message, instance) { return logTo.log(instance + " enter " + element); });
            }
        };
        InitialiseElements.prototype.visitRegion = function (region, deepHistoryAbove) {
            ...
            _super.prototype.visitRegion.call(this, region, deepHistoryAbove || (regionInitial && regionInitial.kind === StateJS.PseudoStateKind.DeepHistory));
            // leave the curent active child state when exiting the region
            this.behaviour(region).leave.push(function (message, stateMachineInstance) { return invoke(_this.behaviour(stateMachineInstance.getCurrent(region)).leave, message, stateMachineInstance); });
            ...
            if (deepHistoryAbove || !regionInitial || regionInitial.isHistory()) {
                this.behaviour(region).endEnter.push(function (message, stateMachineInstance, history) {
                    invoke(_this.behaviour((history || regionInitial.isHistory()) ? stateMachineInstance.getCurrent(region) || regionInitial : regionInitial).enter(), message, stateMachineInstance, history || regionInitial.kind === StateJS.PseudoStateKind.DeepHistory);
                });
            }
            else {
                Array.prototype.push.apply(this.behaviour(region).endEnter, this.behaviour(regionInitial).enter());
            }
            this.addLogging(region, StateJS.logger);
        };
        InitialiseElements.prototype.visitVertex = function (vertex, deepHistoryAbove) {
            this.addLogging(vertex, StateJS.logger);
        };
    ...
    var Validator = (function (_super) {
        ...
        Validator.prototype.visitPseudoState = function (pseudoState) {
            ...
            if (pseudoState.kind === StateJS.PseudoStateKind.Choice || pseudoState.kind === StateJS.PseudoStateKind.Junction) {
                ...
                if (pseudoState.outgoing.length === 0) {
                    StateJS.logger.error(pseudoState + ": " + pseudoState.kind + " pseudo states must have at least one outgoing transition.");
                }
                ...
                if (pseudoState.outgoing.filter(function (transition) { return transition.guard === StateJS.Transition.FalseGuard; }).length > 1) {
                    StateJS.logger.error(pseudoState + ": " + pseudoState.kind + " pseudo states cannot have more than one Else transitions.");
                }
            }
            else {
                ...
                if (pseudoState.outgoing.filter(function (transition) { return transition.guard === StateJS.Transition.FalseGuard; }).length !== 0) {
                    StateJS.logger.error(pseudoState + ": " + pseudoState.kind + " pseudo states cannot have Else transitions.");
                }
                if (pseudoState.isInitial()) {
                    if (pseudoState.outgoing.length !== 1) {
                        // [1] An initial vertex can have at most one outgoing transition.
                        // [2] History vertices can have at most one outgoing transition.
                        StateJS.logger.error(pseudoState + ": initial pseudo states must have one outgoing transition.");
                    }
                    else {
                        ...
                        if (pseudoState.outgoing[0].guard !== StateJS.Transition.TrueGuard) {
                            StateJS.logger.error(pseudoState + ": initial pseudo states cannot have a guard condition.");
                        }
                    }
                }
            }
        ...
        Validator.prototype.visitRegion = function (region) {
            ...
            region.vertices.forEach(function (vertex) {
                if (vertex instanceof StateJS.PseudoState && vertex.isInitial()) {
                    if (initial) {
                        StateJS.logger.error(region + ": regions may have at most one initial pseudo state.");
                    }
                    initial = vertex;
                }
            });
        ...
        Validator.prototype.visitState = function (state) {
            ...
            if (state.regions.filter(function (state) { return state.name === StateJS.Region.defaultName; }).length > 1) {
                StateJS.logger.error(state + ": a state cannot have more than one region named " + StateJS.Region.defaultName);
            }
        };
        Validator.prototype.visitFinalState = function (finalState) {
            ...
            if (finalState.outgoing.length !== 0) {
                StateJS.logger.error(finalState + ": final states must not have outgoing transitions.");
            }
            // [2] A final state cannot have regions.
            if (finalState.regions.length !== 0) {
                StateJS.logger.error(finalState + ": final states must not have child regions.");
            }
            // [4] A final state has no entry behavior.
            if (finalState.entryBehavior.length !== 0) {
                StateJS.logger.warn(finalState + ": final states may not have entry behavior.");
            }
            // [5] A final state has no exit behavior.
            if (finalState.exitBehavior.length !== 0) {
                StateJS.logger.warn(finalState + ": final states may not have exit behavior.");
            }
        };
        Validator.prototype.visitTransition = function (transition) {
            ...
            if (transition.kind === StateJS.TransitionKind.Local) {
                if (ancestors(transition.target).indexOf(transition.source) === -1) {
                    StateJS.logger.error(transition + ": local transition target vertices must be a child of the source composite sate.");
                }
            }
        };
        return Validator;
