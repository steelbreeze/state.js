...
(function (StateJS) {
    ...
    function isActive(element, stateMachineInstance) {
        ...
        else if (element instanceof StateJS.Region) {
...
(function (StateJS) {
    ...
    function isComplete(element, instance) {
        if (element instanceof StateJS.Vertex) {
            return element instanceof StateJS.State ? element.regions.every(function (region) { return isComplete(region, instance); }) : true;
        }
        else if (element instanceof StateJS.Region) {
        ...
        InitialiseTransitions.prototype.visitLocalTransition = function (transition, behaviour) {
            ...
            transition.traverse.push(function (message, instance) {
                ...
                while (i < targetAncestors.length) {
                    _this.cascadeElementEntry(transition, behaviour, targetAncestors[i++], i < targetAncestors.length ? targetAncestors[i] : undefined, function (actions) { invoke(actions, message, instance); });
                }
                // trigger cascade
    ...
    var InitialiseElements = (function (_super) {
        ...
        InitialiseElements.prototype.visitRegion = function (region, deepHistoryAbove) {
            ...
            region.vertices.forEach(function (vertex) {
                vertex.accept(_this, deepHistoryAbove || (region.initial && region.initial.kind === StateJS.PseudoStateKind.DeepHistory));
            });
            ...
            if (deepHistoryAbove || !region.initial || region.initial.isHistory()) {
                regionBehaviour.endEnter.push(function (message, stateMachineInstance, history) {
                    invoke(_this.behaviour((history || region.initial.isHistory()) ? stateMachineInstance.getCurrent(region) || region.initial : region.initial).enter, message, stateMachineInstance, history || region.initial.kind === StateJS.PseudoStateKind.DeepHistory);
                });
            }
            else {
        ...
        InitialiseElements.prototype.visitPseudoState = function (pseudoState, deepHistoryAbove) {
            ...
            if (pseudoState.isInitial()) {
                this.behaviour(pseudoState).endEnter.push(function (message, stateMachineInstance) {
