...
(function (StateJS) {
    ...
    function enter(elementBehavior) {
        return new StateJS.Behavior(elementBehavior.beginEnter).push(elementBehavior.endEnter);
    }
    ...
    var InitialiseTransitions = (function (_super) {
        ...
        InitialiseTransitions.prototype.visitLocalTransition = function (transition, behaviour) {
            ...
            transition.onTraverse.push(function (message, instance) {
                ...
                while (i < targetAncestors.length) {
                    _this.cascadeElementEntry(transition, behaviour, targetAncestors[i++], targetAncestors[i], function (actions) { actions.invoke(message, instance); });
                }
                // trigger cascade
        ...
        InitialiseTransitions.prototype.visitExternalTransition = function (transition, behaviour) {
            ...
            // enter the target ancestry
            while (i < targetAncestors.length) {
                this.cascadeElementEntry(transition, behaviour, targetAncestors[i++], targetAncestors[i], function (actions) { return transition.onTraverse.push(actions); });
            }
            // trigger cascade
    ...
    var InitialiseElements = (function (_super) {
        ...
        InitialiseElements.prototype.visitRegion = function (region, deepHistoryAbove) {
            ...
            this.behaviour(region).leave.push(function (message, stateMachineInstance) { return _this.behaviour(stateMachineInstance.getCurrent(region)).leave.invoke(message, stateMachineInstance); });
            ...
            this.addLogging(region, StateJS.console);
        };
        InitialiseElements.prototype.visitVertex = function (vertex, deepHistoryAbove) {
            this.addLogging(vertex, StateJS.console);
        };
        ...
        InitialiseElements.prototype.visitState = function (state, deepHistoryAbove) {
            ...
            state.regions.forEach(function (region) {
                ...
                _this.behaviour(state).leave.push(_this.behaviour(region).leave);
                _this.behaviour(state).endEnter.push(enter(_this.behaviour(region)));
            });
