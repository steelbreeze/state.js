...
(function (StateJS) {
    ...
    var StateMachine = (function (_super) {
        ...
        function StateMachine(name) {
            ...
            this.logTo = { log: function (message) { } };
            this.warnTo = { warn: function (message) { } };
...
(function (StateJS) {
    ...
    function selectTransition(vertex, stateMachineInstance, message) {
        var _this = this;
        if (vertex instanceof StateJS.PseudoState) {
            ...
            if (vertex.kind === StateJS.PseudoStateKind.Choice) {
                return results.length !== 0 ? results[StateJS.getRandom()(results.length)] : elseResult;
            }
            else if (vertex.kind === StateJS.PseudoStateKind.Junction) {
                if (results.length > 1) {
    ...
    var InitialiseTransitions = (function (_super) {
        ...
        InitialiseTransitions.prototype.visitLocalTransition = function (transition, behaviour) {
            ...
            transition.traverse.push(function (message, instance) {
                ...
                behaviour(instance.getCurrent(targetAncestors[i].getParent())).leave.forEach(function (action) { return action(message, instance); });
        ...
        InitialiseElements.prototype.visitRegion = function (region, deepHistoryAbove) {
            ...
            if (deepHistoryAbove || !region.initial || region.initial.isHistory()) {
                regionBehaviour.endEnter.push(function (message, stateMachineInstance, history) {
                    var h = history || region.initial.kind === StateJS.PseudoStateKind.DeepHistory;
                    _this.behaviour((history || region.initial.isHistory()) ? stateMachineInstance.getCurrent(region) || region.initial : region.initial).enter.forEach(function (action) { return action(message, stateMachineInstance, h); });
                });
            }
            else {
