...
(function (StateJS) {
    ...
    function initialise(stateMachineModel, stateMachineInstance, autoInitialiseModel) {
        ...
        if (stateMachineInstance) {
            ...
            // log as required
            StateJS.logTo.log("initialise " + stateMachineInstance);
            ...
            // log as required
            StateJS.logTo.log("initialise " + stateMachineModel.name);
    ...
    function evaluate(stateMachineModel, stateMachineInstance, message, autoInitialiseModel) {
        if (autoInitialiseModel === void 0) { autoInitialiseModel = true; }
        // log as required
        StateJS.logTo.log(stateMachineInstance + " evaluate " + message);
        // initialise the state machine model if necessary
        if (autoInitialiseModel && stateMachineModel.clean === false) {
            initialise(stateMachineModel);
        }
    ...
    function evaluateState(state, stateMachineInstance, message) {
        ...
        if (result) {
            ...
            if (transitions.length === 1) {
                ...
                StateJS.errorTo.error(state + ": multiple outbound transitions evaluated true for message " + message);
            }
        }
        return result;
    }
    ...
    function selectTransition(pseudoState, stateMachineInstance, message) {
        ...
        if (pseudoState.kind === StateJS.PseudoStateKind.Choice) {
            ...
            if (results.length > 1) {
                StateJS.errorTo.error("Multiple outbound transition guards returned true at " + this + " for " + message);
            }
            else {
    ...
    function findElse(pseudoState) {
        return pseudoState.outgoing.filter(function (transition) { return transition.guard === StateJS.Transition.FalseGuard; })[0]; // NOTE: validator will ensure max one else transition
    }
    ...
    var InitialiseElements = (function (_super) {
        ...
        InitialiseElements.prototype.visitElement = function (element, deepHistoryAbove) {
            if (StateJS.logTo !== defaultConsole) {
                this.behaviour(element).leave.push(function (message, instance) { return StateJS.logTo.log(instance + " leave " + element); });
                this.behaviour(element).beginEnter.push(function (message, instance) { return StateJS.logTo.log(instance + " enter " + element); });
            }
        };
        InitialiseElements.prototype.visitRegion = function (region, deepHistoryAbove) {
            var _this = this;
            var initial = region.vertices.reduce(function (result, vertex) { return vertex instanceof StateJS.PseudoState && vertex.isInitial() ? vertex : result; }, undefined);
            _super.prototype.visitRegion.call(this, region, deepHistoryAbove || (initial && initial.kind === StateJS.PseudoStateKind.DeepHistory));
            ...
            if (deepHistoryAbove || !initial || initial.isHistory()) {
                this.behaviour(region).endEnter.push(function (message, stateMachineInstance, history) {
                    _this.behaviour((history || initial.isHistory()) ? stateMachineInstance.getCurrent(region) || initial : initial).enter().forEach(function (action) { return action(message, stateMachineInstance, history || initial.kind === StateJS.PseudoStateKind.DeepHistory); });
                });
            }
            else {
     ...
     * @member {IErrorTo}
     */
    StateJS.errorTo = defaultConsole;
})(StateJS || (StateJS = {}));
/*
 * Finite state machine library
 * Copyright (c) 2014-5 Steelbreeze Limited
    ...
    var Validator = (function (_super) {
        ...
        Validator.prototype.visitPseudoState = function (pseudoState) {
            ...
            if (pseudoState.kind === StateJS.PseudoStateKind.Choice || pseudoState.kind === StateJS.PseudoStateKind.Junction) {
                ...
                if (pseudoState.outgoing.length === 0) {
                    StateJS.errorTo.error(pseudoState + ": " + pseudoState.kind + " pseudo states must have at least one outgoing transition.");
                }
                ...
                if (pseudoState.outgoing.filter(function (transition) { return transition.guard === StateJS.Transition.FalseGuard; }).length > 1) {
                    StateJS.errorTo.error(pseudoState + ": " + pseudoState.kind + " pseudo states cannot have more than one Else transitions.");
                }
            }
            else {
                ...
                if (pseudoState.outgoing.filter(function (transition) { return transition.guard === StateJS.Transition.FalseGuard; }).length !== 0) {
                    StateJS.errorTo.error(pseudoState + ": " + pseudoState.kind + " pseudo states cannot have Else transitions.");
                }
                if (pseudoState.isInitial()) {
                    if (pseudoState.outgoing.length !== 1) {
                        // [1] An initial vertex can have at most one outgoing transition.
                        // [2] History vertices can have at most one outgoing transition.
                        StateJS.errorTo.error(pseudoState + ": initial pseudo states must have one outgoing transition.");
                    }
                    else {
                        ...
                        if (pseudoState.outgoing[0].guard !== StateJS.Transition.TrueGuard) {
                            StateJS.errorTo.error(pseudoState + ": initial pseudo states cannot have a guard condition.");
                        }
                    }
                }
            }
        ...
        Validator.prototype.visitRegion = function (region) {
            ...
            region.vertices.forEach(function (vertex) {
                if (vertex instanceof StateJS.PseudoState && vertex.isInitial()) {
                    if (initial) {
                        StateJS.errorTo.error(region + ": regions may have at most one initial pseudo state.");
                    }
                    initial = vertex;
                }
            });
        ...
        Validator.prototype.visitState = function (state) {
            ...
            if (state.regions.filter(function (state) { return state.name === StateJS.Region.defaultName; }).length > 1) {
                StateJS.errorTo.error(state + ": a state cannot have more than one region named " + StateJS.Region.defaultName);
            }
        };
        Validator.prototype.visitFinalState = function (finalState) {
            ...
            if (finalState.outgoing.length !== 0) {
                StateJS.errorTo.error(finalState + ": final states must not have outgoing transitions.");
            }
            // [2] A final state cannot have regions.
            if (finalState.regions.length !== 0) {
                StateJS.errorTo.error(finalState + ": final states must not have child regions.");
            }
            // [4] A final state has no entry behavior.
            if (finalState.entryBehavior.length !== 0) {
                StateJS.warnTo.warn(finalState + ": final states may not have entry behavior.");
            }
            // [5] A final state has no exit behavior.
            if (finalState.exitBehavior.length !== 0) {
                StateJS.warnTo.warn(finalState + ": final states may not have exit behavior.");
            }
        };
        Validator.prototype.visitTransition = function (transition) {
            ...
            if (transition.kind === StateJS.TransitionKind.Local) {
                if (ancestors(transition.target).indexOf(transition.source) === -1) {
                    StateJS.errorTo.error(transition + ": local transition target vertices must be a child of the source composite sate.");
                }
            }
        };
        return Validator;
