...
export abstract class NamedElement {
	 ...
	 */
	/*protected*/ constructor(name: string, parent: NamedElement) {
		this.name = name;
...
export class Transition {
	...
	public constructor(source: Vertex, target?: Vertex, kind: TransitionKind = TransitionKind.External) {
		...
		this.guard = source instanceof PseudoState ? Transition.PseudoStateGuard : (message => message === this.source);
...
export abstract class Visitor<TArg1> {
	 ...
	 */
	public visitElement(namedElement: NamedElement, arg1?: TArg1, arg2?: any, arg3?: any): any {
	}
 ...
 * @param instance The state machine instance.
 */
export function isComplete(regionOrState: Region | State, instance: IInstance): boolean {
	if (regionOrState instanceof Region) {
		return instance.getCurrent(regionOrState).isFinal();
	} else {
		return regionOrState.regions.every(region => { return isComplete(region, instance); });
	}
}
 ...
 * @private
 */
class InitialiseTransitions extends Visitor<(namedElement: NamedElement) => Behavior> {
	visitTransition(transition: Transition, behavior: (namedElement: NamedElement) => Behavior) {
	...
	visitInternalTransition(transition: Transition, behavior: (namedElement: NamedElement) => Behavior) {
	...
	visitLocalTransition(transition: Transition, behavior: (element: NamedElement) => Behavior) {
		transition.onTraverse.push((message, instance) => {
	...
	visitExternalTransition(transition: Transition, behavior: (namedElement: NamedElement) => Behavior) {
	...
	cascadeElementEntry(transition: Transition, behavior: (namedElement: NamedElement) => Behavior, vertex: Vertex, next: Vertex, task: (behavior: Array<Action>) => void) {
...
class InitialiseElements extends Visitor<boolean> {
	...
	private behavior(namedElement: NamedElement): Behavior {
		return this.behaviors[namedElement.qualifiedName] || (this.behaviors[namedElement.qualifiedName] = new Behavior());
	}

	visitElement(namedElement: NamedElement, deepHistoryAbove: boolean) {
	...
	visitStateMachine(stateMachine: StateMachine, deepHistoryAbove: boolean) {
		...
		stateMachine.accept(new InitialiseTransitions(), (namedElement: NamedElement) => this.behavior(namedElement));
...
class Validator extends Visitor<string> {
	...
	public visitPseudoState(pseudoState: PseudoState): any {
		...
		if (pseudoState.kind === PseudoStateKind.Choice || pseudoState.kind === PseudoStateKind.Junction) {
			...
			if (pseudoState.outgoing.filter((transition: Transition) => { return transition.guard === Transition.ElseGuard; }).length > 1) {
			...
			if (pseudoState.outgoing.filter((transition: Transition) => { return transition.guard === Transition.ElseGuard; }).length !== 0) {
