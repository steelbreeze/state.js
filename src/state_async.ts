 ...
 */
/** @internal */ async function invoke(actions: Array<Action>, message: any, instance: IInstance, deepHistory: boolean = false) {
 ...
 */
export async function initialise(model: StateMachine, instance?: IInstance, autoInitialiseModel: boolean = true): Promise<void> {
	if (instance) {
 ...
 */
export async function evaluate(model: StateMachine, instance: IInstance, message: any, autoInitialiseModel: boolean = true): Promise<boolean> {
	// initialise the state machine model if necessary
	if (autoInitialiseModel && model.clean === false) {
		await initialise(model);
	}
 ...
 * @returns Returns true if the message caused a [[Transition]].
 */
/** @internal */ async function evaluateState(state: State, instance: IInstance, message: any): Promise<boolean> {
	let result = false;
	...
	if (message !== state) {
		...
		for ( let region in state.regions) {
			...
			if (currentState && (await evaluateState(currentState, instance, message))) {
				result = true;
 ...
 */
/** @internal */ async function traverse(transition: Transition, instance: IInstance, message?: any): Promise<boolean> {
	...
	if (transition.target) {
		...
		if (transition.target instanceof PseudoState && transition.target.kind === PseudoStateKind.Choice) {
			await traverse(selectTransition(transition.target, instance, message), instance, message);
		}
...
/** @internal */ class InitialiseTransitions extends Visitor<(vertexOrRegion: Vertex | Region) => Behavior> {
	async visitTransition(transition: Transition, behavior: (vertexOrRegion: Vertex | Region) => Behavior) {
	...
	async visitLocalTransition(transition: Transition, behavior: (vertexOrRegion: Vertex | Region) => Behavior) {
		transition.onTraverse.push(async (message, instance) => {
			...
			while (i < targetAncestors.length) {
				this.cascadeElementEntry(transition, behavior, targetAncestors[i++], targetAncestors[i], async behavior => await invoke(behavior, message, instance));
			}
...
/** @internal */ class InitialiseElements extends Visitor<boolean> {
	...
	visitRegion(region: Region, deepHistoryAbove: boolean) {
		...
		if (deepHistoryAbove || !regionInitial || regionInitial.isHistory()) { // NOTE: history needs to be determined at runtime
			this.behavior(region).endEnter.push(async (message, instance, deepHistory) => await invoke((this.behavior((deepHistory || regionInitial!.isHistory()) ? instance.getLastKnownState(region) || regionInitial! : regionInitial!)).enter(), message, instance, deepHistory || regionInitial!.kind === PseudoStateKind.DeepHistory));
		} else {
	...
	visitPseudoState(pseudoState: PseudoState, deepHistoryAbove: boolean) {
		...
		if (pseudoState.isInitial()) {
			this.behavior(pseudoState).endEnter.push(async (message, instance, deepHistory) => {
				...
				if ((deepHistory || pseudoState.isHistory()) && (currentState = instance.getLastKnownState(pseudoState.parent))) {
					...
					await invoke(this.behavior(currentState).enter(), message, instance, deepHistory || pseudoState.kind === PseudoStateKind.DeepHistory);
				} else {
			...
			this.behavior(pseudoState).beginEnter.push(async (message, instance) => { instance.isTerminated = true; return; });
		}
	}
